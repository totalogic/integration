...
output: 
    # The first Output file is the one it creates, should see if case can be used
    ...
    html_document:
        #Table of Contents
        toc: yes
        # More bling for the TOC
        toc_float:
            # TOC is highlighted where I am
            smooth_scrolling: yes
            collapsed: no
        # this is a fuunction of html_document
        number_sections: yes
...
To put in an equation you have to start and stop with 
...
$$
    \bar{X} = \sum_{i=1}^n \frac{1}{n} x_1
$$

Another option with R Markdown is the inline method but its only one `$`
...
RStudio Connect will house various types of apps and whatnot, shiny or other


You can use rstudio connect to render multiple versions based on parameter

Oh yeah he forgot flexdashboards

```{r InstallFlexDashboards}
